...
:: ============================================================================
:: PREREQUISITE AUDIT & AUTOMATED DEPENDENCY INSTALLATION
:: ============================================================================
...
:: ============================================================================
:: DEPLOY PRE-COMPILED STANDALONE BINARIES
:: ============================================================================
echo [2/6] Deploying application binaries to %TARGET_DIR%...
echo [%DATE% %TIME%] Deploying binaries to %TARGET_DIR%... >> "%LOG_FILE%"

if exist "%SCRIPT_DIR%dist\MPCustom.Service.exe" (
    xcopy "%SCRIPT_DIR%dist\*" "%TARGET_DIR%\" /E /Y /Q >> "%LOG_FILE%" 2>&1
) else if exist "%SCRIPT_DIR%MPCustom.Service.exe" (
    xcopy "%SCRIPT_DIR%*" "%TARGET_DIR%\" /E /Y /Q >> "%LOG_FILE%" 2>&1
) else (
    echo [INFO] Compiling local binaries for deployment...
    dotnet publish "%SCRIPT_DIR%MPCustom.sln" -c Release -o "%TARGET_DIR%" --nologo >> "%LOG_FILE%" 2>&1
)
...
:: ============================================================================
:: FIREWALL & DOMAIN PROTECTION INITIALIZATION
:: ============================================================================
...
:: ============================================================================
:: START MENU SHORTCUT & INITIAL LAUNCH
:: ============================================================================
